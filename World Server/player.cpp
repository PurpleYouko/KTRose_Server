...
{
    if( Ride->Ride && !Ride->Drive ) //I'm a passenger in somebody else's cart
    {
        CPlayer* otherclient = GServer->GetClientByCID( Ride->charid, Position->Map ); //find the cart's driver
        if( otherclient != NULL ) // found the driver
        {
            if( otherclient->Status->Stance != 0x04 ) //not driving any more
            {
                ...
                Log(MSG_DEBUG,"(UpdateValues) not in cart any more. Destiny set to current position X: %f Y: %f.",Position->current.x,Position->current.y);
            }
            else //still driving
            {
                Position->current = otherclient->Position->current;
                Position->destiny = otherclient->Position->destiny;
                Log(MSG_DEBUG,"(UpdateValues) Passenger in Cart Destiny set to current position X: %f Y: %f.",Position->current.x,Position->current.y);
                Position->lastMoveTime = otherclient->Position->lastMoveTime;
                return false;  // will not update the player position
            }
        }
        else  //no driver found so reset values to NOT be a passenger any more
        {
            Ride->Drive = false;
            Ride->Ride = false;
            Ride->charid = 0;
            Position->destiny = Position->current;
            Log(MSG_DEBUG,"(UpdateValues) No other client. Destiny set to current position X: %f Y: %f.",Position->current.x,Position->current.y);
        }
    }
    return true;
}
...
*/
  ...
  //  else
    ...
    ADDWORD( pak, Stats->Mspd_base );			    // WALK SPEED MAYBE?
	ADDBYTE( pak, 0x00 );										// ??
	ADDBYTE( pak, 0x00 );										// ??
	ADDBYTE( pak, 0x01 );										// ??
