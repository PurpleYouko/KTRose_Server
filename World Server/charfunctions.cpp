...
void CCharacter::StartAction( CCharacter* Target, BYTE action, UINT skillid)
{
    ...
    switch(action)
    {
        case NORMAL_ATTACK:
        {
            ...
            Battle->bufftarget = 0;
            Battle->skillid = 0;
            Position->destiny  = Target->Position->current; //move to intercept target
            Position->lastMoveTime = clock();
            //Log(MSG_DEBUG,"Normal Attack set destiny X: %f Y: %f.",Position->current.x,Position->current.y);
        }
        break;
        ...
        case SKILL_BUFF:
        {
            ...
            Battle->atktype = action;
            Position->destiny  = Target->Position->current;  //move toward target
            Battle->skillid = skillid;
            Position->lastMoveTime = clock();
            //Log(MSG_DEBUG,"Skill Attack set destiny X: %f Y: %f.",Position->current.x,Position->current.y);
        }
        break;
        ...
        case BUFF_AOE:
        {
            ...
            Battle->atktype = action;
            Battle->skillid = skillid;
            Position->skilltargetpos.x = Position->current.x;
            Position->skilltargetpos.y = Position->current.y;
...
bool CCharacter::CanAttack( )
{
    ...
    if(Battle->iscasting == 1)
    {
        if( etime < CLOCKS_PER_SEC * GServer->Config.SkillMult / Stats->Attack_Speed ) //should make the delay 4 times longer
        {
        ...
        {
            Log(MSG_INFO,"iscasting reset to 0 after time delay");
            Battle->iscasting = 0;
            Battle->NextAtkTime = Battle->lastAtkTime + (CLOCKS_PER_SEC * GServer->Config.SkillMult / Stats->Attack_Speed);
            return true;
        }
    }
    else //normal attack
    {
        if( etime < CLOCKS_PER_SEC * GServer->Config.AttkMult / Stats->Attack_Speed )
        {
            //Log(MSG_INFO,"Cannot attack yet. Last attack time = %i",Battle->lastAtkTime);
            return false;
        }
...
bool CCharacter::IsTargetReached( CCharacter* Enemy, CSkills* skill )
{
    ...
    if(skill == NULL) //it's a normal attack
    {
        if(distance <= Stats->Attack_Distance)
        {
            ...
            //Log(MSG_DEBUG,"(istargetreached) Destiny set to current position X: %f Y: %f.",Position->current.x,Position->current.y);
            return true;
        }
        else
        {
            Position->destiny = Enemy->Position->current;
            //Log(MSG_DEBUG,"(istargetreached) Destiny set to Enemy position X: %f Y: %f.",Position->current.x,Position->current.y);
        }
    }
    else // skill attack
    {
        float SkillRange = (float)skill->range;
        if(SkillRange == 0)
        {
            SkillRange = Stats->Attack_Distance;
        }
        if(distance <= SkillRange)
        {
            Position->destiny = Position->current;
            //Log(MSG_DEBUG,"(istargetreached skill) Destiny set to current position X: %f Y: %f.",Position->current.x,Position->current.y);
            return true;
        }
        else
        {
            Position->destiny = Enemy->Position->current;
            //Log(MSG_DEBUG,"(istargetreached skill) Destiny set to Enemy position X: %f Y: %f.",Position->current.x,Position->current.y);
            //Log(MSG_DEBUG,"(istargetreached skill) Distance: %f range: %i.", distance, SkillRange);
        }
    }
    //Log(MSG_INFO,"(istargetreached) Cannot attack. Too far away");
    return false;
}
...
void CCharacter::RefreshBuff( )
{
    ...
    for( UINT i=0;i<30;i++)
    {
        ...
        else if (MagicStatus[i].Status >= 7 && MagicStatus[i].Status <= 11 && etime > 1*CLOCKS_PER_SEC) //Do poison dmg every 1.5 seconds
        {
             ...
             {
                 //printf("char died\n");
                 Log( MSG_INFO, "found dead monster");
                 CDrop* thisdrop = NULL;
                 ...
                 //kill the monster if its HP is less than 0
                 BEGINPACKET( pak1, 0x799 );
                 ADDWORD    ( pak1, clientid );
                 ADDWORD    ( pak1, clientid );
                 ADDDWORD   ( pak1, 0x8005 );
