...
unsigned int CMonster::GetDefense( )
{
    ...
    if(defense == 0)defense = 1; //hopefully stops some division by zero errors.
    return defense;    
}
...
unsigned int CMonster::GetMoveSpeed( )
{
    ...
    if (Stats->stance == 0) //mWalking. All other stances produce running animation at client
    {
...
unsigned int CMonster::GetDodge( )
{
    ...
    if(dodge == 0)dodge = 1; //hopefully stops some division by zero errors.                                                                 
    return dodge;
}
...
unsigned int CMonster::GetMagicDefense( )
{
    ...
    if(mdef == 0)mdef = 1; //hopefully stops some division by zero errors.                                                                 
    return mdef;
}
