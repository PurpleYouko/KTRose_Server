...
{
    CMonster* monster = AddMonster( newmontype, npc->pos, 0, 0, 1 );
...
bool CMap::TeleportPlayer( CPlayer* player, fPoint coord, bool TelePassenger )
{
    ...
    Log(MSG_DEBUG,"(TeleportPlayer) Destiny set to target position X: %f Y: %f.",player->Position->current.x,player->Position->current.y);
...
bool CMap::pakTeleport( CPlayer* player, unsigned int Map, fPoint coord, bool TelePassenger )
{
    ...
    player->Position->current = coord;
    player->Position->destiny = coord;
    Log(MSG_DEBUG,"(pakTeleport) Destiny set to target position X: %f Y: %f.",player->Position->current.x,player->Position->current.y);
    player->Session->inGame = false;
    player->Position->lastMoveTime = clock();
    if(!allowpat || !TelePassenger)
    {
...
void CMap::UpdateTime( )
{
    ...
    int Hourlength = int(0xffffffff / 24);
    int Minutelength = int(Hourlength / 60);
    ...
    hour = int(MapTime / Hourlength);
    minute = int((MapTime - (hour * Hourlength)) / Minutelength);
    LastUpdate = clock();
}
        ...
        for (UINT j = 0; j < MobGroupList.size(); j++)
        {
            ...
            for (UINT k = thisgroup->active; k < thisgroup->limit; k++)
            {
                ...
                for (UINT i = 0; i < thismob->amount; i++)
                {
                    ...
                    AddMonster( thismob->mobId, position, 0, thisgroup->id, 1 );
                }
            }
        }
    }
...
CCharacter* CMap::GetCharInMap( unsigned int id )
{
    ...
    //Log(MSG_DEBUG, "getcharinmap %i", id);
    if(monster != NULL)
    {
        //Log(MSG_DEBUG, "Monster found type = %i", monster->montype);
