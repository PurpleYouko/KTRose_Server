...
class CWorldServer : public CServerSocket
{
    public:
        ...
        CDrop* GetNewDrop( CMonster* thismon );
        CDrop* GetPYDrop( CMonster* thismon, UINT droptype );
    	void SendToAll( CPacket* pak );
    	UINT GetNewClientID( );
    	void DisconnectAll();
    	CPlayer* GetClientByUserID( UINT userid );
    	UINT GetMatIDfromRecipe (UINT CraftLevel, UINT matType);
        UINT GetFairyRange( UINT part );
        void RefreshFairy( );
        inline dword GetStorageFee(dword iBasePrice, dword iPriceRate, dword uiDupCnt) {return ((((iBasePrice*5) / 0x3E8)+1)*(iPriceRate+1)) * uiDupCnt;};
    	...
    	bool pakGMHeal( CPlayer* thisclient, int hpamount, int mpamount );
    	...
    	bool pakGMMon( CPlayer* thisclient, int montype, int moncount, int monAI );
        ...
        bool pakGMNpc(CPlayer* thisclient, int npcid, int dialogid);
      	...
      	bool pakGMEffect( CPlayer* thisclient, int effect);
      	...
      	bool pakGMUnionPoints(CPlayer* thisclient, char* name, int nb_points);
        ...
        bool SendGlobalMSG( CPlayer* thisclient, const char* Format, ... );
        //bool CWorldServer::NPCShout( CMonster* thismon, char msg[200], char npc[50] );
        bool NPCShout( int thismap, const char msg[200], const char npc[50] );
        bool NPCMessage( CMonster* thismon, const char msg[200], const char npc[50] );
        bool NPCWhisper( CPlayer* thisclient, CMonster* thismon, const char msg[200], const char npc[50] );
        bool NPCAnnounce( const char msg[200], const char npc[50] );
        ...
        //vector<CAIPData*>       AIPData;                // AIP filename list
        ...
        vector<CAip*> AipList;
        //vector<CAIPScript*> AIPList;
        UINT maxSpawnId;
        fpAiCond aiCondFunc[35];
        fpAiAct aiActFunc[40];
        int ObjVar[2000][20];                           //NPC variables used in AI
        UINT ThisSession;                               //session ID from database
        UINT ServerStartTime;                           //time the server was started
        UINT ServerRunTime;                             //Hours the server has been running
        CLTBstring Ltbstring[200];
        ...
        //Test thingies
        unsigned ATTK_SPEED_MOD;
        unsigned HIT_DELAY_MOD;
        unsigned MOVE_SPEED_MOD;
