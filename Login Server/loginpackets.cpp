...
{
     Log(MSG_DEBUG,"User Login request received");
     try
     {
    	...
    	if( mysql_num_rows( result ) == 1 )
        {
    		...
    		if ( res == 0 )
            {
    			...
    			{ //The server are under inspection
                    Log(MSG_WARNING,"User accesslevel %i is lower than minimum %i",thisclient->accesslevel,Config.MinimumAccessLevel);
                    ADDBYTE( pak, 0 );
                    ADDDWORD( pak, 0 );
                    thisclient->SendPacket( &pak );
                    DB->QFree( );
    			...
    			if ( thisclient->accesslevel > 0 )
                {
    				...
    				DB->QFree( );
    				//OK!
    				Log(MSG_WARNING,"User %s passed to charserver",thisclient->username.c_str());
                ...
                {
    				//BANNED
    				Log(MSG_WARNING,"User %s banned",thisclient->username.c_str());
    				ADDBYTE( pak, 5 );
    				ADDDWORD( pak, 0 );
    				DB->QFree( );
    			}
            ...
            {
    			//BAD PASSWORD
    			Log(MSG_WARNING,"User %s attempted to log in with invalid password",thisclient->username.c_str());
    			ADDBYTE( pak, 3 );
    			ADDDWORD( pak, 0 );
    			DB->QFree( );
    		}
