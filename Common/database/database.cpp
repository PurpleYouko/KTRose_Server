...
int CDatabase::Connect( )
{
  ...
  MYSQL_RES *myresult = QStore("SELECT @@wait_timeout");
...
MYSQL_RES* CDatabase::QStore( const char *Format, ...)
{
    ...
    Log( MSG_QUERY, query );
    //Log( MSG_INFO, query );
    //Log( MSG_INFO, "Locking pthread mutex" );
    pthread_mutex_lock( &SQLMutex );
    //Log( MSG_INFO, "Locked pthread mutex" );
    while(Qfail)
    {
        if(mysql_query( Mysql, query )!=0)
        {
    ...
    //Log( MSG_INFO, "reading mysql result" );
    result = mysql_store_result( Mysql );
    //Log( MSG_INFO, "returning mysql result" );
    return result;
}
